...
#---------------------------------------------------------------------------------------
env.project_name = 'chamados'
env.server_name = 'telefonia.cmc.pr.gov.br'
env.webapps_root = '/usr/share/webapps/'
env.webapps_git = 'chamadas-telefonia.git'

env.project_root = os.path.join(env.webapps_root, env.project_name)
env.activate_script = os.path.join(env.project_root, '/usr/share/webapps/chamados/bin')
env.wsgi_file = os.path.join(env.project_root, 'django.wsgi')
env.repo_root = os.path.join(env.project_root, env.webapps_git)
...
cd /usr/share/webapps/chamados/bin
source activate
cd /usr/share/webapps/chamados/chamadas-telefonia.git
exec gunicorn chamados.wsgi:application --workers $NUM_WORKERS --bind 127.0.0.1:8888 --user=$USER --group=$GROUP --log-level error --log-file=$LOGFILE 2>>$LOGFILE
